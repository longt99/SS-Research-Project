...
def process_data(goes_data, stix_data, swaves_data1, swaves_data2, wind_data1, wind_data2, ilofar_data):
    ...
    '''
    Converts data from variables to Timeseries and Spectrogram objects for plotting
    '''
    ...
    # finding the upper and lower limits for normalisation for WAVES and S/WAVES
...
def summary_plot(date, start_time, end_time, goes, stix, swaves_spec1, wind_spec1, sw_data, sw_freq, wind_data, wind_freq, lims_sw, lims_w, ilofar_spec, lims_i):  
    ...
    '''
    Creates a summary plot of the data which takes the date, start and end times,
    and assigned data as arguments. The function also takes the argument "instrument"
    which specifies which spectrogram to include, the default being WAVES
    '''
    
    # Creating a time object from start and end time args
    ...
    # Adding in a for loop which depends on arg instrument which indicates whether or
    # not to use waves or s/waves... didn't work for S/WAVES so needs to be added in (02-OCT-21)
    
    #if instrument == 'swaves':
...
#         axes[0].set_title('S/WAVES', y=1.01, pad=-14, bbox=box, loc='left', fontsize='12')
...
#     axes[0].set_ylabel('MHz', fontsize='14')
#     axes[0].set_yscale('log')
#     axes[0].set_title('S/WAVES', y=1.01, pad=-14, bbox=box, loc='left', fontsize='14')
#     axes[0].tick_params(which='major', bottom=False, top=False, left=True, right=False, direction='out', labelsize='10', length=5)
    ...
    #axes[3].xaxis.set_major_locator(plt.MaxNLocator(10))
