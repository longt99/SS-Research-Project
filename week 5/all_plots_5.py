...
#                 continue
            ...
            print('There was a problem with %s', inst)
...
def fetch_data(start_time, end_time, query):
    ...
    '''
    Fetches and assigns data to variables from query depending on whether or not
    the data is available and creates a summary plot based on this.
    '''
    ...
    # Defining the text box features for adding to summary plots
    ...
    if ((len(query[4][0]) == 0) & (len(query[1][0]) == 0)):
        ...
        # WIND/WAVES and S/WAVES data, the former is preferred
...
#         ax.xaxis.set_major_formatter(date_format)
    ...
    # If WIND/WAVES returns no results
...
#         ax.xaxis.set_major_formatter(date_format)
    ...
    # If I-LOFAR returns no results
        ...
        wind_data = np.concatenate((wind_spec1.data, wind_spec2.data), axis=0)
        wind_freq = np.concatenate((wind_spec1.frequencies, wind_spec2.frequencies), axis=0)
        lims_w = np.percentile(wind_data.data, (3, 99))
...
#         ax.xaxis.set_major_formatter(date_format)
    ...
    # If results are obtained for all instruments 
...
#         ax.xaxis.set_major_formatter(date_format)
    ...
    # Now, setting some parameters common to all summary plots
    ...
    fig.savefig("/Users/thomas/SS Research Project/week 5/event_plots/"+str(start_time_obj)[:10]+"_4.png")
    #plt.show()
...
multiple_summary_plots('13:20:00', '13:40:00', queries[:]) #'00:00:00', '23:59:59'
plt.show()
