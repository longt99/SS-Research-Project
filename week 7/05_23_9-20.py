...
radio_query = Fido.search(a.Time('2021-05-23T09:00', '2021-05-23T10:00'), 
...
axes.plot(datetime(2021, 5, 23, 9, 18, 29, 250000), 80, color='black', marker='X', ms='10')

axes.plot(datetime(2021, 5, 23, 9, 18, 30, 500000), 45.5, color='black', marker='X', ms='10')

# order not guarnteed so need to set plot range

axes.set_xlim(datetime(2021, 5, 23, 9, 18, 15), datetime(2021, 5, 23, 9, 19, 0))
#axes.set_ylim(16,70)
...
#     axes.plot(time_s, spec, color=c, label=label)
    ...
    ax.set_xlim(time_s[300], time_s[-800])
...
cpd_file = Fido.fetch(stix_query[0][-7])
...
popt, pcov = curve_fit(power_law, e_center[2:17].value, counts[2:17].value, 
                       sigma=count_errs[2:17].value)
...
plt.plot(e_center[2:17], power_law(e_center[2:17].value, *popt), color='red', linestyle='--', linewidth=2, label='Fit to STIX Science Data')
...
#fig.savefig("/Users/thomas/SS Research Project/week 7/events/spectrum_"+str(query[0][0][0])[:10]+"_"+str(query[0][0][0])[11:16]+".png")
