...
radio_query = Fido.search(a.Time('2021-09-23T04:00', '2021-09-23T06:00'), 
...
axes.plot(datetime(2021, 9, 23, 4, 58, 27, 750000), 58, color='black', marker='X', ms='10')
axes.plot(datetime(2021, 9, 23, 4, 58, 31, 250000), 20, color='black', marker='X', ms='10')

# order not guarnteed so need to set plot range

axes.set_xlim(datetime(2021, 9, 23, 4, 58, 10), datetime(2021, 9, 23, 4, 58, 55))
axes.set_ylim(16,70)
#axes.set_yscale('log')
axes.invert_yaxis()
...
stix_query = Fido.search(a.Time('2021-09-23 00:00:00', '2021-09-23 23:59:59'), a.Instrument.stix,
...
cpd_file = Fido.fetch(stix_query[0][0])
...
popt, pcov = curve_fit(power_law, e_center[2:15].value, counts[2:15].value, 
                       sigma=count_errs[2:15].value)
...
plt.plot(e_center[2:15], power_law(e_center[2:15].value, *popt), color='red', linestyle='--', linewidth=2, label='Fit to STIX Science Data')
