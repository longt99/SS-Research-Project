...
radio_query = Fido.search(a.Time('2021-05-23T16:00', '2021-05-23T18:00'), 
...
axes.plot(datetime(2021, 5, 23, 17, 1, 31, 250000), 80, color='black', marker='X', ms='10')

axes.plot(datetime(2021, 5, 23, 17, 1, 33), 45.5, color='black', marker='X', ms='10')

# order not guarnteed so need to set plot range

axes.set_xlim(datetime(2021, 5, 23, 17, 1, 20), datetime(2021, 5, 23, 17, 2, 0))
#axes.set_ylim(16,70)
...
#     axes.plot(time_s, spec, color=c, label=label)
    ...
    ax.set_xlim(time_s[0], time_s[1500])
...
cpd_file = Fido.fetch(stix_query[0][-4])
...
popt, pcov = curve_fit(power_law, e_center[2:12].value, counts[2:12].value, 
                       sigma=count_errs[2:12].value)
