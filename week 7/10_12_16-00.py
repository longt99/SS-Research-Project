...
radio_query = Fido.search(a.Time("2021/10/12 00:00:00", "2021/10/12 23:59:59"), a.Instrument('ILOFAR'))
...
# +
# Plotting I-LOFAR data over which the STIX data will be plotted for comparison

ilofar_files = Fido.fetch(radio_query[0][-2])
ilofar_spec = Spectrogram(sorted(ilofar_files))
lims_i = [np.percentile(s.data, (5, 99.8)) for s in ilofar_spec]


fig, axes = plt.subplots(3, 1, figsize=(8.,10.), sharex=True)

[spec.plot(axes=ax, norm=LogNorm(vmin=l[0], vmax=l[1])) for spec, l, ax in zip(ilofar_spec, lims_i, axes)]

s_time = '2021-10-12 15:58:00'
e_time = '2021-10-12 16:02:00'
    
start_t = datetime.strptime(s_time,'%Y-%m-%d %H:%M:%S')
end_t = datetime.strptime(e_time, '%Y-%m-%d %H:%M:%S')
...
# stix = axes.twinx()
# data.plot_timeseries(axes=stix, energy_indices=[3,4,5], error_bar=False)
# stix.set_yscale('linear')
# #stix.legend()
...
fig.text(0.07,0.55, "Frequency (MHz)", ha="center", va="center", rotation=90, fontsize='12')

#plt.subplots_adjust(wspace=0, hspace=0)

fig.savefig("/Users/thomas/SS Research Project/week 7/"+str(radio_query[0][0][0])[:10]+"_"+str(radio_query[0][0][0])[11:16]+".png")
...
# +
goes_query = Fido.search(a.Time('2021-10-12 00:00', '2021-10-12 23:59'), a.Instrument("XRS"), a.goes.SatelliteNumber(16))
goes_file = Fido.fetch(goes_query)

goes_plot = ts.TimeSeries(goes_file)

fig = plt.figure(figsize = (10,7))

plt.plot(goes_plot.index, (np.gradient(goes_plot.data['xrsa'])/np.max(np.gradient(goes_plot.data['xrsa']))**2))#/np.max(np.gradient(plot.data['xrsa'])))
#plt.plot(plot.index[39500:41000], np.gradient(plot.data['xrsa'][39500:41000])/np.max(np.gradient(plot.data['xrsa'][39500:41000])))



# plt.xlim('2021-09-23 15:25:00', '2021-09-23 15:35:00')
plt.xlim('2021-10-12 15:56:00', '2021-10-12 16:03:00')
...
# +
time_s = ilofar_spec[0].times.datetime
#time_s

set1 = ilofar_spec[0].data[140:160]
set2 = ilofar_spec[1].data[50:80]
set3 = ilofar_spec[1].data[120:150]
set4 = ilofar_spec[2].data[20:40]
set5 = ilofar_spec[2].data[60:80]


freq_labels = ['$65-73\;MHz$', '$130-141\;MHz$', '$157-169\;MHz$', '$218-226\;MHz$', '$234-241\;MHz$' ]

time_series1 = set1.sum(axis=0)
time_series2 = set2.sum(axis=0)
time_series3 = set3.sum(axis=0)
time_series4 = set4.sum(axis=0)
time_series5 = set5.sum(axis=0)

spectra = [time_series1,time_series2,time_series3,time_series4,time_series5]
...
fig, axes = plt.subplots(6,1, sharex=True, figsize=(9,12), gridspec_kw={'height_ratios': [3, 1, 1, 1, 1, 1]})

[spec.plot(axes=axes[0], norm=LogNorm(vmin=l[0], vmax=l[1])) for spec, l in zip(ilofar_spec, lims_i)]
...
#     axes.plot(time_s, spec, color=c, label=label)
    ...
    ax.set_xlim(time_s[2150], time_s[-1350])
...
stix_query = Fido.search(a.Time('2021-05-22 00:00:00', '2021-05-22 23:59:59'), a.Instrument.stix,
...
cpd_file = Fido.fetch(stix_query[0][14])
...
popt, pcov = curve_fit(power_law, e_center[2:13].value, counts[2:13].value, 
                       sigma=count_errs[2:13].value)
...
plt.plot(e_center[2:13], power_law(e_center[2:13].value, *popt), color='red', linestyle='--', linewidth=2, label='Fit to STIX Science Data')
