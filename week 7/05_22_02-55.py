...
radio_query = Fido.search(a.Time('2021-05-22T02:00', '2021-05-22T03:15'), 
...
lims = [np.percentile(s.data, (3, 97)) for s in callisto_specs]
...
axes.plot(datetime(2021, 5, 22, 2, 54, 48), 75, color='black', marker='X', ms='10')
axes.plot(datetime(2021, 5, 22, 2, 54, 51), 30, color='black', marker='X', ms='10')

# order not guarnteed so need to set plot range

axes.set_xlim(datetime(2021, 5, 22, 2, 54, 30), datetime(2021, 5, 22, 2, 55, 10))
axes.set_ylim(10,90)
...
# plt.xlim('2021-05-22 02:53:00', '2021-05-22 02:58:00')
# #plt.yscale('log')
# plt.ylim(0,0.1)
...
#     axes.plot(time_s, spec, color=c, label=label)
    ...
    ax.set_xlim(time_s[1900], time_s[3100])
...
axes[4].plot(goes_plot.index, 1*(np.gradient(goes_plot.data['xrsb'])/np.max(np.gradient(goes_plot.data['xrsb']))), color='black')#/np.max(np.gradient(goes_plot.data['xrsa']))))#/np.max(np.gradient(plot.data['xrsa'])))
...
cpd_file = Fido.fetch(stix_query[0][9])
...
bg_data = stix_cpd.get_data(time_indices=[tind_min],
                            ...
                            energy_indices=list(range(1,28)), # use all requested channels
...
flare_data = stix_cpd.get_data(time_indices=[tind_max], # this will sum counts 0-4 time
                               energy_indices=list(range(1,28)),
#                                energy_indices=einds,
                               detector_indices=[[0,31]],  # sum all detectors
                               pixel_indices=[[0,11]])  # sum all pixels
...
popt, pcov = curve_fit(power_law, e_center[2:14].value, counts[2:14].value, 
...
plt.plot(e_center[2:14], power_law(e_center[2:14].value, *popt), color='red', linestyle='--', linewidth=2, label='Fit to STIX Science Data')
