...
radio_query = Fido.search(a.Time('2021-05-22T16:45', '2021-05-22T17:45'), 
...
axes.plot(datetime(2021, 5, 22, 17, 9, 37, 500000), 80, color='black', marker='X', ms='10')
axes.plot(datetime(2021, 5, 22, 17, 9, 39, 250000), 45.5, color='black', marker='X', ms='10')

# order not guarnteed so need to set plot range

axes.set_xlim(datetime(2021, 5, 22, 17, 9, 10), datetime(2021, 5, 22, 17, 10, 10))
#axes.set_ylim(16,70)
...
#     axes.plot(time_s, spec, color=c, label=label)
    ...
    ax.set_xlim(time_s[1800], time_s[-800])
...
cpd_file = Fido.fetch(stix_query[0][25])
