...
radio_query = Fido.search(a.Time('2021-09-23T15:00', '2021-09-23T16:00'), 
...
axes.plot(datetime(2021, 9, 23, 15, 30, 31, 500000), 80, color='black', marker='X', ms='10')
axes.plot(datetime(2021, 9, 23, 15, 30, 32, 750000), 45.5, color='black', marker='X', ms='10')

# order not guarnteed so need to set plot range

axes.set_xlim(datetime(2021, 9, 23, 15, 30, 10), datetime(2021, 9, 23, 15, 30, 50))
#axes.set_ylim(16,70)
...
#     axes.plot(time_s, spec, color=c, label=label)
    ...
    ax.set_xlim(time_s[1200], time_s[2000])
...
cpd_file = Fido.fetch(stix_query[0][2])
