...
radio_query = Fido.search(a.Time('2021-05-22T11:00', '2021-05-22T12:00'), 
...
axes.plot(datetime(2021, 5, 22, 11, 18, 26, 250000), 45.5, color='black', marker='X', ms='10')
axes.plot(datetime(2021, 5, 22, 11, 18, 24, 500000), 80, color='black', marker='X', ms='10')

# order not guarnteed so need to set plot range

axes.set_xlim(datetime(2021, 5, 22, 11, 18, 10), datetime(2021, 5, 22, 11, 18, 50))
#axes.set_ylim(16,70)
...
#     axes.plot(time_s, spec, color=c, label=label)
    ...
    ax.set_xlim(time_s[600], time_s[1900])
    ax.legend(frameon=False, fontsize='12', loc='upper right')
    ax.tick_params(which='both', bottom=True, top=True, left=True, right=True, direction='in', labelsize='12', length=5)


axes[4].plot(goes_plot.index, 5*(np.gradient(goes_plot.data['xrsb'])/np.max(np.gradient(goes_plot.data['xrsb']))), color='black', label='GOES SXR Derivative $1-8\;\AA$')#/np.max(np.gradient(goes_plot.data['xrsa']))))#/np.max(np.gradient(plot.data['xrsa'])))
...
axes[4].legend(frameon=False, fontsize='12', loc='lower left')
