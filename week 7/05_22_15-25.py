...
radio_query = Fido.search(a.Time('2021-05-22T15:00', '2021-05-22T16:00'), 
...
lims = [np.percentile(s.data, (1, 98)) for s in callisto_specs]
...
axes.plot(datetime(2021, 5, 22, 15, 24, 26, 250000), 80, color='black', marker='X', ms='10')
axes.plot(datetime(2021, 5, 22, 15, 24, 28, 250000), 45.5, color='black', marker='X', ms='10')

# order not guarnteed so need to set plot range

axes.set_xlim(datetime(2021, 5, 22, 15, 23, 55), datetime(2021, 5, 22, 15, 24, 55))
#axes.set_ylim(16,70)

#axes.set_yscale('log')
axes.invert_yaxis()

axes.set_xlabel('Time (UT) '+str(radio_query[0][0][0])[:10], fontsize='12')
...
# +
goes_query = Fido.search(a.Time('2021-05-22 00:00', '2021-05-22 23:59'), a.Instrument("XRS"), a.goes.SatelliteNumber(16))
goes_file = Fido.fetch(goes_query)

goes_plot = ts.TimeSeries(goes_file)

# +
stix_ql = Fido.search(a.Time('2021-05-22 00:00:00', '2021-05-22 23:59:59'), a.Instrument.stix)
stix_ql_file = Fido.fetch(stix_ql[0][0])

stix_ql_plot = ts.TimeSeries(stix_ql_file)
# -
...
time_s = callisto_specs[1].times.datetime
#time_s

# three frequency bands, summed "row-wise" for timeseries plotting

set1 = callisto_specs[1].data[40:60]
set2 = callisto_specs[1].data[90:110]
set3 = callisto_specs[1].data[140:160]
...
fig, axes = plt.subplots(6,1, sharex=True, figsize=(9,15), gridspec_kw={'height_ratios': [2, 1, 1, 1, 2, 2]})

callisto_specs[1].plot(axes=axes[0], vmin=lims[1][0], vmax=lims[1][1])
...
#     axes.plot(time_s, spec, color=c, label=label)
    ...
    #ax.set_xlim(time_s[2000], time_s[-1000])
...
axes[4].plot(goes_plot.index, 3.33*(np.gradient(goes_plot.data['xrsb'])/np.max(np.gradient(goes_plot.data['xrsb']))), color='black', label='GOES SXR Derivative $1-8\;\AA$')#/np.max(np.gradient(goes_plot.data['xrsa']))))#/np.max(np.gradient(plot.data['xrsa'])))

axes[4].set_ylim(-0.7,1.5)
axes[4].tick_params(which='both', bottom=True, top=True, left=True, right=True, direction='in', labelsize='12', length=5)
axes[4].legend(frameon=False, fontsize='12', loc='upper left')


stix_ql_plot.plot(axes=axes[5])

date_format = mdates.DateFormatter('%H:%M')
axes[5].set_title('')
axes[5].xaxis.set_major_formatter(date_format)
axes[5].xaxis.set_tick_params(rotation=0, labelbottom=True)
axes[5].set_xlabel('Time (UT)', fontsize='12')
axes[5].legend(frameon=False, fontsize='12', loc='upper left')

#axes[2].set_ylabel('Normalised Intensity (arb. units)', fontsize='12')
plt.xticks(ha='center')

fig.text(0.072,0.55, "Normalised Intensity (arb. units)", ha="center", va="center", rotation=90, fontsize='12')
fig.text(0.072,0.8, "Frequency (MHz)", ha="center", va="center", rotation=90, fontsize='12')
...
cpd_file = Fido.fetch(stix_query[0][16])
