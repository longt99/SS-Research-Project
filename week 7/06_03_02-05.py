...
radio_query = Fido.search(a.Time('2021-06-03T01:00', '2021-06-03T03:00'), 
...
index = ((radio_query['CALLISTO']['Observatory'] == 'Australia-ASSA') )
...
lims = [np.percentile(s.data, (2, 98.5)) for s in callisto_specs]
...
axes.plot(datetime(2021, 6, 3, 2, 3, 32, 0), 60, color='black', marker='X', ms='10')
axes.plot(datetime(2021, 6, 3, 2, 3, 36, 500000), 25, color='black', marker='X', ms='10')


# order not guarnteed so need to set plot range

axes.set_xlim(datetime(2021, 6, 3, 2, 3, 20), datetime(2021, 6, 3, 2, 3, 50))
axes.set_ylim(10, 90)
...
# +
goes_query = Fido.search(a.Time('2021-06-03 00:00', '2021-06-03 23:59'), a.Instrument("XRS"), a.goes.SatelliteNumber(16))
...
freq_labels = ['$64.81-71.94\;MHz$', '$46.06-53.19\;MHz$', '$27.31-34.44\;MHz$']
...
fig, axes = plt.subplots(5,1, sharex=True, figsize=(9,12), gridspec_kw={'height_ratios': [2, 1, 1, 1, 1]})
...
#     axes.plot(time_s, spec, color=c, label=label)
    ...
    ax.set_xlim(time_s[300], time_s[1800])
...
#axes[4].plot(goes_plot.index, 100*(np.gradient(goes_plot.data['xrsb'])/np.max(np.gradient(goes_plot.data['xrsb'])))**2, color='black')#/np.max(np.gradient(goes_plot.data['xrsa']))))#/np.max(np.gradient(plot.data['xrsa'])))
axes[4].plot(goes_plot.index, 5*(np.gradient(goes_plot.data['xrsb'])/np.max(np.gradient(goes_plot.data['xrsb']))), color='black')#/np.max(np.gradient(goes_plot.data['xrsa']))))#/np.max(np.gradient(plot.data['xrsa'])))
#axes[4].plot

p = sp.polyfit(goes_plot.index, 5*(np.gradient(goes_plot.data['xrsb'])/np.max(np.gradient(goes_plot.data['xrsb']))), deg=50)
y_ = sp.polyval(p, goes_plot.index)

# plot smoothened data
axes[4].plot(goes_plot.index, y_, color='r', linewidth=2)
...
axes[4].xaxis.set_tick_params(rotation=0, labelbottom=True)
...
stix_query = Fido.search(a.Time('2021-06-03 00:00:00', '2021-06-03 23:59:59'), a.Instrument.stix,
...
cpd_file = Fido.fetch(stix_query[0][1])
...
bg_data = stix_cpd.get_data(time_indices=[tind_min],
                            ...
                            energy_indices=list(range(1,14)), # use all requested channels
...
flare_data = stix_cpd.get_data(time_indices=[tind_max], # this will sum counts 0-4 time
                               energy_indices=list(range(1,14)),
#                                energy_indices=einds,
                               detector_indices=[[0,31]],  # sum all detectors
                               pixel_indices=[[0,11]])  # sum all pixels
