...
radio_query = Fido.search(a.Time('2021-08-29T6:00', '2021-08-29T19:00'), 
...
lims = [np.percentile(s.data, (5, 99)) for s in callisto_specs]
...
axes.set_xlim(datetime(2021, 8, 29, 17, 0, 0), datetime(2021, 8, 29, 19, 0, 0))
#axes.set_ylim(10, 90)
...
#     axes.plot(time_s, spec, color=c, label=label)
    ...
    ax.set_xlim(time_s[300], time_s[2000])
...
axes[4].plot(goes_plot.index, 100*(np.gradient(goes_plot.data['xrsa'])/np.max(np.gradient(goes_plot.data['xrsb'])))**2, color='black')#/np.max(np.gradient(goes_plot.data['xrsa']))))#/np.max(np.gradient(plot.data['xrsa'])))
axes[4].set_ylim(0,1)
