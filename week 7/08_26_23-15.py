...
radio_query = Fido.search(a.Time('2021-08-26T23:00', '2021-08-26T23:45'), 
...
#, color='black', marker='X', ms='10'

axes.plot(datetime(2021, 8, 26, 23, 21, 10, 750000), 60, color='black', marker='X', ms='10')
axes.plot(datetime(2021, 8, 26, 23, 21, 13, 250000), 25, color='black', marker='X', ms='10')

# order not guarnteed so need to set plot range

axes.set_xlim(datetime(2021, 8, 26, 23, 20, 45), datetime(2021, 8, 26, 23, 22, 0))
axes.set_ylim(10, 90)
...
#     axes.plot(time_s, spec, color=c, label=label)
    ...
    ax.set_xlim(time_s[1200], time_s[2400])
...
stix_query = Fido.search(a.Time('2021-08-26 00:00:00', '2021-08-26 23:59:59'), a.Instrument.stix,
...
bg_data = stix_cpd.get_data(time_indices=[tind_min],
                            ...
                            energy_indices=list(range(1,23)), # use all requested channels
...
flare_data = stix_cpd.get_data(time_indices=[tind_max], # this will sum counts 0-4 time
                               energy_indices=list(range(1,23)),
#                                energy_indices=einds,
                               detector_indices=[[0,31]],  # sum all detectors
                               pixel_indices=[[0,11]])  # sum all pixels
