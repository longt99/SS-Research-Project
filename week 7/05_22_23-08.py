...
radio_query = Fido.search(a.Time('2021-05-22T22:45', '2021-05-22T23:15'), 
...
lims = [np.percentile(s.data, (4, 97.5)) for s in callisto_specs]
...
axes.plot(datetime(2021, 5, 22, 23, 8, 40, 0), 80, color='black', marker='X', ms='10')
axes.plot(datetime(2021, 5, 22, 23, 8, 43, 750000), 30, color='black', marker='X', ms='10')

# order not guarnteed so need to set plot range

axes.set_xlim(datetime(2021, 5, 22, 23, 8, 25), datetime(2021, 5, 22, 23, 9, 0))
axes.set_ylim(10,90)
...
#     axes.plot(time_s, spec, color=c, label=label)
    ...
    ax.set_xlim(time_s[1800], time_s[2500])
...
cpd_file = Fido.fetch(stix_query[0][24])
...
bg_data = stix_cpd.get_data(time_indices=[tind_min],
                            ...
                            energy_indices=list(range(1,21)), # use all requested channels
...
flare_data = stix_cpd.get_data(time_indices=[tind_max], # this will sum counts 0-4 time
                               energy_indices=list(range(1,21)),
#                                energy_indices=einds,
                               detector_indices=[[0,31]],  # sum all detectors
                               pixel_indices=[[0,11]])  # sum all pixels
