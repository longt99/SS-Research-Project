...
radio_query = Fido.search(a.Time('2021-08-28T04:00', '2021-08-28T06:00'), 
...
lims = [np.percentile(s.data, (3, 99)) for s in callisto_specs]
...
axes.plot(datetime(2021, 8, 28, 5, 4, 27, 500000), 63, color='black', marker='X', ms='10')
axes.plot(datetime(2021, 8, 28, 5, 4, 29, 500000), 38, color='black', marker='X', ms='10')

# order not guarnteed so need to set plot range

axes.set_xlim(datetime(2021, 8, 28, 5, 4, 10), datetime(2021, 8, 28, 5, 4, 50))
axes.set_ylim(10,90)
...
#     axes.plot(time_s, spec, color=c, label=label)
    ...
    ax.set_xlim(time_s[600], time_s[1500])
...
stix_query = Fido.search(a.Time('2021-08-28 00:00:00', '2021-08-28 23:59:59'), a.Instrument.stix,
