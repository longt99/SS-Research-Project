...
radio_query = Fido.search(a.Time('2021-05-23T10:00', '2021-05-23T12:00'), 
...
lims = [np.percentile(s.data, (2, 98)) for s in callisto_specs]
...
axes.plot(datetime(2021, 5, 23, 11, 4, 23, 500000), 80, color='black', marker='X', ms='10')
axes.plot(datetime(2021, 5, 23, 11, 4, 25, 500000), 55.5, color='black', marker='X', ms='10')
...
# +
goes_query = Fido.search(a.Time('2021-05-23 00:00', '2021-05-23 23:59'), a.Instrument("XRS"), a.goes.SatelliteNumber(16))
...
#     axes.plot(time_s, spec, color=c, label=label)
    ...
    ax.set_xlim(time_s[0], time_s[2500])
...
axes[4].plot(goes_plot.index, (np.gradient(goes_plot.data['xrsa'])))#/np.max(np.gradient(goes_plot.data['xrsa']))))#/np.max(np.gradient(plot.data['xrsa'])))
axes[4].set_ylim(-0.5e-7,0.5e-7)
...
stix_query = Fido.search(a.Time('2021-05-23 00:00:00', '2021-05-23 23:59:59'), a.Instrument.stix,
...
cpd_file = Fido.fetch(stix_query[0][-5])
...
bg_data = stix_cpd.get_data(time_indices=[tind_min],
                            ...
                            energy_indices=list(range(1,20)), # use all requested channels
...
flare_data = stix_cpd.get_data(time_indices=[tind_max], # this will sum counts 0-4 time
                               energy_indices=list(range(1,20)),
#                                energy_indices=einds,
                               detector_indices=[[0,31]],  # sum all detectors
                               pixel_indices=[[0,11]])  # sum all pixels
