...
radio_query = Fido.search(a.Time('2021-09-25T03:00', '2021-09-25T04:00'), 
...
lims = [np.percentile(s.data, (3, 98.5)) for s in callisto_specs]
...
axes.plot(datetime(2021, 9, 25, 3, 50, 28, 0), 60, color='black', marker='X', ms='10')
axes.plot(datetime(2021, 9, 25, 3, 50, 34, 500000), 20, color='black', marker='X', ms='10')

# order not guarnteed so need to set plot range

axes.set_xlim(datetime(2021, 9, 25, 3, 50, 15), datetime(2021, 9, 25, 3, 50, 50))
axes.set_ylim(10, 90)
...
#     axes.plot(time_s, spec, color=c, label=label)
    ...
    ax.set_xlim(time_s[800], time_s[2000])
...
stix_query = Fido.search(a.Time('2021-09-25 00:00:00', '2021-09-25 23:59:59'), a.Instrument.stix,
...
cpd_file = Fido.fetch(stix_query[0][6])
...
tind_min = stix_cpd.data['counts'].sum(axis=(1,2,3))[:74].argmin()

stix_cpd.energy_masks
# -

len(stix_cpd.data['counts'].sum(axis=(1,2,3)))

einds

# +
# get background data (need to chose this away from any flare)
bg_data = stix_cpd.get_data(time_indices=[tind_min-1],
...
plt.plot(e_center[2:12], power_law(e_center[2:12].value, *popt), color='red', linestyle='--', linewidth=2, label='Fit to STIX Science Data')
