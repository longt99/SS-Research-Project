...
from radiospectra import sources
from radiospectra import spectrogram
from radiospectra import spectrum
from radiospectra.sources import SWavesSpectrogram
...
def summary_plot(start, end):
    ...
    sw_read1 = SWavesSpectrogram.read(swaves_data1[0])
    lims_sw_l = [np.percentile(swaves_spec_l.data, (3, 98))]
    
    
    swaves_data2 = Fido.fetch(query['swaves'][2])
    swaves_spec_h = Spectrogram(sorted(swaves_data2))
    
    sw_read2 = SWavesSpectrogram.read(swaves_data2[0])
    lims_sw_h = [np.percentile(swaves_spec_h.data, (3, 98))]
    ...
    axes[4].text(ilofar_spec[0].times.datetime[150], 35, 'I-LOFAR',color='black', size='12', bbox=box)
    
    wind_spec[0].plot(axes=axes[1], norm=LogNorm(vmin=lims_w[0][0], vmax=lims_w[0][1]))
    axes[1].text(ilofar_spec[0].times.datetime[150], 120, 'WIND R1',color='black', size='12', bbox=box)
    wind_spec[1].plot(axes=axes[2], norm=LogNorm(vmin=lims_w[1][0], vmax=lims_w[1][1]))
    axes[2].text(ilofar_spec[0].times.datetime[150], 2, 'WIND R2',color='black', size='12', bbox=box)
    
    swaves_spec_h.plot(axes=axes[3], vmin = np.percentile(sw_read1[0], 4), vmax = np.percentile(sw_read1[0], 96))
    axes[3].text(ilofar_spec[0].times.datetime[150], 2000, 'STEREO WAVES HFR',color='black', size='12', bbox=box)
    swaves_spec_l.plot(axes=axes[0], vmin = np.percentile(sw_read2[2], 4), vmax = np.percentile(sw_read2[2], 96))
    axes[0].text(ilofar_spec[0].times.datetime[150], 20, 'STEREO WAVES LFR',color='black', size='12', bbox=box)
    ...
    axes[5].grid(axis='y')
    axes[5].text(ilofar_spec[0].times.datetime[150], 7*10**(-7), 'GOES XRS',color='black', size='12', bbox=box)
    ...
    axes[6].set_ylim(5,10**(3.5))
    axes[6].text(ilofar_spec[0].times.datetime[150], 2*10**(3), 'STIX',color='black', size='12', bbox=box)
    
    fig.suptitle('Summary Plot for '+ start_day_sample, fontsize='18')
    plt.subplots_adjust(wspace=0, hspace=0)
    fig.subplots_adjust(top=0.95)
    #fig.savefig("/Users/thomas/SS Research Project/week 2/sum_plt.png")
    plt.show()
